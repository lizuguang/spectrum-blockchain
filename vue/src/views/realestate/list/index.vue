<template>
  <div class="container">
    ...
    >
      ...
      <p>用户名: {{ userName }}</p>
      <p>余额: ￥{{ balance }} 元</p>
      <p>当发起出租、转让或质押操作后，担保状态为true</p>
      <p>当担保状态为false时，才可发起出租、转让或质押操作</p>
    </el-alert>
    ...
    <el-row v-loading="loading" :gutter="20">
      <el-col v-for="(val,index) in realEstateList" :key="index" :span="6" :offset="1">
        <el-card class="realEstate-card">
          ...
          <div class="item">
            <el-tag type="info">频率范围: </el-tag>
            <span>{{ val.minfrequency }} - {{ val.maxfrequency }} Hz</span>
          </div>
          <div class="item">
            <el-tag type="warning">授权时间: </el-tag>
            <span>{{ val.startdate }} 至 {{ val.enddate }}</span>
          </div>

          <div v-if="!val.encumbrance&&roles[0] !== 'admin'">
            <el-button type="text" @click="openDialog(val)">出租</el-button>
            <el-divider direction="vertical" />
            <el-button type="text" @click="openDonatingDialog(val)">转让</el-button>
          </div>
    ...
    <el-dialog v-loading="loadingDialog" :visible.sync="dialogCreateDonating" :close-on-click-modal="false" @close="resetForm('DonatingForm')">
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="createDonating('DonatingForm')">立即转让</el-button>
