...
func CreateSelling(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	if realEstate.Encumbrance {
		return shim.Error("此频谱资源已经作为担保状态，不能重复发起出租")
	}
	selling := &lib.Selling{
...
func CreateSellingByBuy(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	//根据objectOfSale和seller获取想要购买的频谱信息，确认存在该频谱
	resultsRealEstate, err := utils.GetStateByPartialCompositeKeys2(stub, lib.RealEstateKey, []string{seller, objectOfSale})
	if err != nil || len(resultsRealEstate) != 1 {
		return shim.Error(fmt.Sprintf("根据%s和%s获取想要购买的频谱信息失败: %s", objectOfSale, seller, err))
	}
	//根据objectOfSale和seller获取出租信息
	...
	if err != nil || len(resultsSelling) != 1 {
		return shim.Error(fmt.Sprintf("根据%s和%s获取出租信息失败: %s", objectOfSale, seller, err))
	}
	...
	if selling.SellingStatus != lib.SellingStatusConstant()["saleStart"] {
		return shim.Error("此交易不属于出租中状态，已经无法购买")
	}
	//根据buyer获取买家信息
	...
	if buyerAccount.Balance < selling.Price {
		return shim.Error(fmt.Sprintf("频谱售价为%f,您的当前余额为%f,购买失败", selling.Price, buyerAccount.Balance))
	}
...
// QuerySellingList 查询出租(可查询所有，也可根据发起出租人查询)(发起的)(供卖家查询)
...
// QuerySellingListByBuyer 根据参与出租人、买家(买家AccountId)查询出租(参与的)(供买家查询)
...
// UpdateSelling 更新出租状态（买家确认、买卖家取消）
func UpdateSelling(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	case "done":
		...
		selling.ObjectOfSale = realEstate.RealEstateID //重新更新频谱ID
