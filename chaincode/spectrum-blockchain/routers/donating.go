...
import (
	...
	"github.com/lizuguang/spectrum-blockchain/chaincode/spectrum-blockchain/utils"
	"time"
)
...
func CreateDonating(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	if donor == grantee {
		return shim.Error("转让人和接收人不能同一人")
	}
	...
	if err != nil || len(resultsAccount) != 1 {
		return shim.Error(fmt.Sprintf("grantee接收人信息验证失败%s", err))
	}
	...
	if accountGrantee.UserName == "管理员" {
		return shim.Error(fmt.Sprintf("不能转让给管理员%s", err))
	}
	//判断记录是否已存在，不能重复发起转让
	//若Encumbrance为true即说明此频谱已经正在担保状态
	if realEstate.Encumbrance {
		return shim.Error("此频谱资源已经作为担保状态，不能再发起转让")
	}
	...
	if err := utils.WriteLedger(donatingGrantee, stub, lib.DonatingGranteeKey, []string{donatingGrantee.Grantee, fmt.Sprintf("%d", createTimeUnixNano.UnixNano())}); err != nil {
		return shim.Error(fmt.Sprintf("将本次转让交易写入账本失败%s", err))
	}
...
// QueryDonatingList 查询转让列表(可查询所有，也可根据发起转让人查询)(发起的)(供转让人查询)
...
// QueryDonatingListByGrantee 根据接收人(接收人AccountId)查询转让(接收的)(供接收人查询)
func QueryDonatingListByGrantee(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	if len(args) != 1 {
		return shim.Error(fmt.Sprintf("必须指定接收人AccountId查询"))
	}
...
func UpdateDonating(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	//根据objectOfDonating和donor获取想要购买的频谱信息，确认存在该频谱
	resultsRealEstate, err := utils.GetStateByPartialCompositeKeys2(stub, lib.RealEstateKey, []string{donor, objectOfDonating})
	if err != nil || len(resultsRealEstate) != 1 {
		return shim.Error(fmt.Sprintf("根据%s和%s获取想要购买的频谱信息失败: %s", objectOfDonating, donor, err))
	}
	var realEstate lib.RealEstate
	if err = json.Unmarshal(resultsRealEstate[0], &realEstate); err != nil {
	...
	if err = json.Unmarshal(resultsGranteeAccount[0], &accountGrantee); err != nil {
		return shim.Error(fmt.Sprintf("查询grantee接收人信息-反序列化出错: %s", err))
	}
	//根据objectOfDonating和donor和grantee获取转让信息
	...
	if err != nil || len(resultsDonating) != 1 {
		return shim.Error(fmt.Sprintf("根据%s和%s和%s获取出租信息失败: %s", objectOfDonating, donor, grantee, err))
	}
	var donating lib.Donating
	if err = json.Unmarshal(resultsDonating[0], &donating); err != nil {
		return shim.Error(fmt.Sprintf("UpdateDonating-反序列化出错: %s", err))
	}
	//不管完成还是取消操作,必须确保转让处于转让中状态
	if donating.DonatingStatus != lib.DonatingStatusConstant()["donatingStart"] {
		return shim.Error("此交易并不处于转让中，确认/取消转让失败")
	}
	...
	if err != nil || len(resultsDonatingGrantee) == 0 {
		return shim.Error(fmt.Sprintf("根据%s获取接收人信息失败: %s", grantee, err))
	}
	...
	case "done":
		...
		realEstate.RealEstateID = fmt.Sprintf("%d", time.Now().Local().UnixNano()) //重新更新频谱ID
		if err := utils.WriteLedger(realEstate, stub, lib.RealEstateKey, []string{realEstate.Proprietor, realEstate.RealEstateID}); err != nil {
			return shim.Error(fmt.Sprintf("%s", err))
		}
		//清除原来的频谱信息
		...
		donating.ObjectOfDonating = realEstate.RealEstateID //重新更新频谱ID
		...
		if err != nil {
			return shim.Error(fmt.Sprintf("序列化转让交易的信息出错: %s", err))
		}
		break
	case "cancelled":
		//重置频谱信息担保状态
