...
func CreateRealEstate(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	startdate := args[4]
	enddate := args[5]
	if accountId == "" || proprietor == "" || minfrequency == "" || maxfrequency == "" || startdate == "" || enddate == "" {
	...
	// 参数数据格式转换
	var formattedMinFrequency float64
	if val, err := strconv.ParseFloat(minfrequency, 64); err != nil {
		return shim.Error(fmt.Sprintf("minfrequency参数格式转换出错: %s", err))
	} else {
		formattedMinFrequency = val
	}
	var formattedMaxFrequency float64
	if val, err := strconv.ParseFloat(maxfrequency, 64); err != nil {
		return shim.Error(fmt.Sprintf("maxfrequency参数格式转换出错: %s", err))
	} else {
		formattedMaxFrequency = val
	}
	//判断是否管理员操作
	...
	realEstate := &lib.RealEstate{
		...
		Encumbrance:  false,
		MinFrequency:  formattedMinFrequency,
		MaxFrequency:  formattedMaxFrequency,
		StartDate:   startdate,
		EndDate:   enddate,
	}
	// 写入账本
...
// QueryRealEstateList 查询频谱资源(可查询所有，也可根据所有人查询拥有的频谱资源)
